...
# unused CONFIGURATIONS_PATH_SPACE := $(CURRENT_DIR_SPACE)/Configurations
...
#                            $(shell ls -1 $(BOARD_PORT) > $(BUILDS_PATH)/serial.txt)
                ...
                #     unused BACK_ADDRESS = $(shell ifconfig | grep "inet " | grep -v 127.0.0.1 | cut -d\ -f 2-)
...
ifeq ($(BOOL_SELECT_BOARD),1)
    ...
    ifndef CORE_LIBS_LIST
        WORK_25 = $(subst .h,,$(subst $(CORE_LIB_PATH)/,,$(wildcard $(CORE_LIB_PATH)/*.h $(CORE_LIB_PATH)/*/*.h)))
        WORK_25 += $(subst .hpp,,$(subst $(CORE_LIB_PATH)/,,$(wildcard $(CORE_LIB_PATH)/*.hpp $(CORE_LIB_PATH)/*/*.hpp)))
        CORE_LIBS_LIST = $(subst $(CORE_LIB_PATH)/,,$(filter-out $(EXCLUDE_LIST),$(WORK_25)))
    ...
    #    WORK_21 = $(filter-out %main.cpp, $(wildcard $(CORE_LIB_PATH)/*.cpp $(CORE_LIB_PATH)/*/*.cpp $(CORE_LIB_PATH)/*/*/*.cpp $(CORE_LIB_PATH)/*/*/*/*.cpp))
        WORK_21a = $(shell find $(CORE_LIB_PATH) -name \*.cpp)

        WORK_21b = $(filter-out %/main.cpp,$(WORK_21a))
        CORE_CPP_SRCS = $(filter-out %/$(EXCLUDE_LIST),$(WORK_21b))
    ...
    ifeq ($(APP_LIBS_LIST),)
    #    WORK_21 = $(realpath $(sort $(dir $(wildcard $(APP_LIB_PATH)/*/*.h $(APP_LIB_PATH)/*/*/*.h))))
    #    WORK_21 += $(realpath $(sort $(dir $(wildcard $(APP_LIB_PATH)/*/*.hpp $(APP_LIB_PATH)/*/*/*.hpp))))
        WORK_21 = $(realpath $(sort $(dir $(shell find $(APP_LIB_PATH) -name \*.h -o -name \*.hpp))))
        APP_LIBS_LIST = $(subst $(APP_LIB_PATH)/,,$(filter-out $(EXCLUDE_LIST),$(WORK_21)))
    endif # APP_LIBS_LIST

    ifeq ($(APP_LIBS_LOCK),)
    ifndef APP_LIBS
    ifneq ($(strip $(APP_LIBS_LIST)),0)
        WORK_24 = $(patsubst %,$(APP_LIB_PATH)/%,$(APP_LIBS_LIST))
    #        APP_LIBS = $(realpath $(sort $(dir $(foreach dir,$(WORK_24),$(wildcard $(dir)/*.h $(dir)/*/*.h $(dir)/*/*/*.h $(dir)/*.hpp $(dir)/*/*.hpp $(dir)/*/*/*.hpp)))))
        APP_LIBS = $(realpath $(sort $(dir $(foreach dir,$(WORK_24),$(shell find $(dir) -name \*.h -o -name \*.hpp)))))
    endif # APP_LIBS_LIST
    endif # APP_LIBS
    endif # APP_LIBS_LOCK
    ...
    ifndef USER_LIBS_LIST
        WORK_22 = $(realpath $(sort $(dir $(wildcard $(USER_LIB_PATH)/*/*.h))))
        WORK_22 += $(realpath $(sort $(dir $(wildcard $(USER_LIB_PATH)/*/*.hpp))))
    #    WORK_22 = $(realpath $(sort $(dir $$(find $(USER_LIB_PATH) -name \*.h))))
    #    WORK_22 += $(realpath $(sort $(dir $$(find $(USER_LIB_PATH) -name \*.hpp))))
        USER_LIBS_LIST = $(subst $(USER_LIB_PATH)/,,$(filter-out $(EXCLUDE_LIST),$(WORK_22)))
    ...
    #    WORK_23 = $(patsubst %,$(USER_LIB_PATH)/%,$(USER_LIBS_LIST))
    #    EXCLUDE_LIST_GREP = $(shell echo $(strip $(EXCLUDE_PATHS)) | sed "s/ /|/g" )
    #    USER_LIBS := $(sort $(foreach dir,$(WORK_23),$(shell find $(dir) -type d | egrep -v '$(EXCLUDE_LIST_GREP)' )))

    # Horrible mess caused by non-compliant libraries
    # 1. Same procedure as for application libraries
        WORK_23 = $(foreach dir,$(USER_LIB_PATH),$(patsubst %,$(dir)/%,$(USER_LIBS_LIST)))
        WORK_23 += $(foreach dir,$(USER_LIB_PATH),$(patsubst %,$(dir)/%/utility,$(USER_LIBS_LIST)))
        WORK_23 += $(foreach dir,$(USER_LIB_PATH),$(patsubst %,$(dir)/%/src,$(USER_LIBS_LIST)))

    # 2. But parse all sub-folders below src for compliant libraries
        WORK_23a = $(foreach dir,$(USER_LIB_PATH),$(patsubst %,$(dir)/%/src,$(USER_LIBS_LIST)))
        WORK_23b := $(foreach dir,$(WORK_23a),$(shell if [ -d $(dir) ] ; then find $(dir) -type d ; fi))
        WORK_23 += $(WORK_23b)

        WORK_23 += $(foreach dir,$(USER_LIB_PATH),$(patsubst %,$(dir)/%/src/utility,$(USER_LIBS_LIST)))

    #     WORK_23c = $(addsuffix /,$(WORK_23))
    #     USER_LIBS := $(filter-out $(EXCLUDE_LIST),$(WORK_23c))

        EXCLUDE_LIST_GREP = $(shell echo $(strip $(EXCLUDE_PATHS)) | sed "s/ /|/g" )
        USER_LIBS := $(sort $(shell echo $(WORK_23) | egrep -v '$$(EXCLUDE_LIST_GREP)' ))

        USER_LIB_CPP_SRC = $(foreach dir,$(USER_LIBS),$(wildcard $(dir)/*.cpp))
        USER_LIB_C_SRC = $(foreach dir,$(USER_LIBS),$(wildcard $(dir)/*.c))
        # unused USER_LIB_H_SRC = $(foreach dir,$(USER_LIBS),$(wildcard $(dir)/*.h))
        # unused USER_LIB_H_SRC += $(foreach dir,$(USER_LIBS),$(wildcard $(dir)/*.hpp))

    #    USER_LIB_CPP_SRC = $(wildcard $(patsubst %,%/*.cpp,$(USER_LIBS)))
    #    USER_LIB_C_SRC = $(wildcard $(patsubst %,%/*.c,$(USER_LIBS)))
    #    # unused USER_LIB_H_SRC = $(wildcard $(patsubst %,%/*.h,$(USER_LIBS)))
    ...
    #
    #     WORK_20a := $(shell find $(LOCAL_LIB_PATH) -maxdepth 1 -type d)
    #     WORK_20b := $(subst ./,,$(WORK_20a))
    #     WORK_20c := $(filter-out $(WORK_20b),$(LOCAL_LIBS_LIST))
    #     $(info WORK_20a $(WORK_20a))
    #     $(info WORK_20b $(WORK_20b))
    #     $(info WORK_20c $(WORK_20c))
    #
    # ifneq ($(strip $(WORK_20c)),)
    #     $(error Missing local libraries $(WORK_20c))
    # endif
    #
    #     $(info .)
    #     $(error STOP)
    ...
    ifndef LOCAL_LIBS_LIST
        # WORK_26 = $(dir $(shell find $(LOCAL_LIB_PATH) -path "*/examples/*" ! -prune -o -name \*.h -o -name \*.hpp))
        WORK_26 = $(dir $(shell find $(LOCAL_LIB_PATH) -type d \( -name examples -o -name tests \) -prune -o \( -name \*.h -o -name \*.hpp \) -print))
        s212 = $(sort $(subst $(LOCAL_LIB_PATH)/,,$(filter-out $(EXCLUDE_LIST)/,$(sort $(WORK_26)))))
        LOCAL_LIBS_LIST = $(shell echo $(s212)' ' | sed 's://:/:g' | sed 's:/ : :g')
        s213 = $(s212)
    endif # LOCAL_LIBS_LIST

    ifneq ($(strip $(LOCAL_LIBS_LIST)),0)
        WORK_27 = $(patsubst %,$(LOCAL_LIB_PATH)/%,$(LOCAL_LIBS_LIST))
    #     WORK_28 = $(sort $(dir $(foreach dir,$(WORK_27),$(shell find $(dir) -name \*.h -o -name \*.hpp))))
        # WORK_28 = $(dir $(foreach dir,$(WORK_27),$(shell find $(dir) -path "*/examples/*" ! -prune -o -name \*.h -o -name \*.hpp)))
        WORK_28 = $(dir $(foreach dir,$(WORK_27),$(shell find $(dir) -type d \( -name examples -o -name tests \) -prune -o \( -name \*.h -o -name \*.hpp \) -print)))
    #     s213 = $(subst $(LOCAL_LIB_PATH)/,,$(filter-out $(EXCLUDE_LIST)/,$(sort $(WORK_27))))
        s213 = $(sort $(subst $(LOCAL_LIB_PATH)/,,$(filter-out $(EXCLUDE_LIST)/,$(WORK_27))))
        s214 = $(shell echo $(WORK_28)' ' | sed 's://:/:g' | sed 's:/ : :g')
    ...
    #
    WORK_29 = $(wildcard $(patsubst %,%/*.cpp,$(LOCAL_LIBS))) $(wildcard $(LOCAL_LIB_PATH)/*.cpp)
    LOCAL_CPP_SRCS = $(filter-out %/$(PROJECT_NAME_AS_IDENTIFIER).cpp, $(WORK_29))
    ...
    #      Filter
        ...
        INFO_LOCAL_UNARCHIVES_LIST = $(sort $(filter-out .builds $(INFO_LOCAL_ARCHIVES_LIST),$(LOCAL_LIBS_LIST_TOP)))
    else
        LOCAL_OBJS = $(RAW_LOCAL_OBJS)
        INFO_LOCAL_UNARCHIVES_LIST = $(sort $(filter-out .builds,$(LOCAL_LIBS_LIST_TOP)))
    endif # USE_ARCHIVES
...
#
SHELL_STARTCHRONO = $(shell $(UTILITIES_PATH)/emCode_chrono $(BUILDS_PATH))
SHELL_STOPCHRONO = $(shell $(UTILITIES_PATH)/emCode_chrono $(BUILDS_PATH) -s)
...
ifneq ($(MESSAGE_CRITICAL),)
    ...
    $(call MESSAGE_GUI_WARNING,$(MESSAGE_CRITICAL))
...
ifeq ($(BOOL_SELECT_BOARD),1)
    ...
    ifneq ($(MESSAGE_WARNING),)
        $(call MESSAGE_GUI_WARNING,$(MESSAGE_WARNING))
...
#     endif # INFO_LOCAL_ARCHIVES_LIST
    ...
    ifneq ($(strip $(LOCAL_LIBS_LIST)),) # all
        ...
        # WORK_20a := $(shell find $(LOCAL_LIB_PATH) -maxdepth 1 -type d)
        WORK_20a := $(shell find $(LOCAL_LIB_PATH) -type d)
        WORK_20b := $(subst ./,,$(WORK_20a))
        WORK_20c := $(filter-out $(WORK_20b),$(LOCAL_LIBS_LIST))

        ifneq ($(strip $(WORK_20c)),)
            $(info Missing folders   $(WORK_20c))
            $(info .)
...
# unused TARGETS = $(OBJDIR)/$(BINARY_SPECIFIC_NAME).*
...
# unused SYS_OBJS = $(wildcard $(patsubst %,%/*.o,$(APP_LIBS)))
# unused SYS_OBJS += $(wildcard $(patsubst %,%/*.o,$(BUILD_APP_LIBS)))
# unused SYS_OBJS += $(wildcard $(patsubst %,%/*.o,$(USER_LIBS)))
...
#	 	$(QUIET_UPLOAD)$(COMMAND_UPLOAD)
        ...
        ifneq ($(filter $(BOARD_TAG),yun),)
            ...
            ifneq ($(MAKECMDGOALS),debug)
				...
				$(shell cd $(CURRENT_DIR); $(UTILITIES_PATH)/uploader_ssh.sh $(SSH_ADDRESS) $(SSH_PASSWORD) $(REMOTE_FOLDER) $(BINARY_SPECIFIC_NAME) -exec)
            endif # MAKECMDGOALS debug

        else ifeq ($(BOARD_TAG),izmir_ec_yocto_MCU)
			$(call SHOW,"10.40-UPLOAD",$(UPLOADER))

			$(shell cd $(CURRENT_DIR); export FIRMWARE_TOOLS_PATH=$(UTILITIES_PATH) ; export SSH_USER=root ; export SSH_IP_ADDR=$(SSH_ADDRESS) ; export SSH_PASSWORD=$(SSH_PASSWORD) ; bash $(UTILITIES_PATH)/download.sh install $(OBJDIR))
			echo $(OBJDIR)

			$(shell cd $(CURRENT_DIR); $(UTILITIES_PATH)/uploader_mcu.sh $(SSH_ADDRESS) $(SSH_PASSWORD) /lib/firmware $(OBJDIR)/intel_mcu.bin "$(MCU_CONFIGURATION)")
		...
		$(call MESSAGE_GUI_INFO,"Click OK and plug the Digispark board into the USB port.")
...
fast: start_message change_main before_compile compile after_compile reset raw_upload
	@echo "==== $(MESSAGE_TASK) done ===="
